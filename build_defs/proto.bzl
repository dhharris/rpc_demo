...
def _get_proto_gen_srcs(lang: str, src: Artifact) -> list[str]:
    ...
    if lang == "cpp":
        return ["{}.pb.cc".format(source_file)]
    elif lang == "py":
        return [
...
def _proto_gencode(ctx: AnalysisContext) -> list[Provider]:
    lang = ctx.attrs.lang
    srcs = []
    headers = []
    ...
    cmd = "protoc -I {} --{}_out $OUT".format(ctx.attrs.path, lang)
    return gencode(
        ctx,
...
def proto_library(name, srcs, languages, deps = [], visibility = [], **kwargs):
